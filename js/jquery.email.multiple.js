            ...
            $element.keydown(function (e) {
                ...
                if (e.keyCode === 13 || e.keyCode === 32) {
                    ...
                    if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(getValue)){
                        $('.all-mail').append('<span class="email-ids">' + getValue + '<span class="cancel-email"><i class="las la-times"></i></span></span>');
                        $element.val('');
            ...
            if(settings.data){
                $.each(settings.data, function (x, y) {
                    if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(y)){
                        $('.all-mail').append('<span class="email-ids">' + y + '<span class="cancel-email"><i class="las la-times"></i></span></span>');
                        $element.val('');
